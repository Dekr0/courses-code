...
void Simulator::start() {
    ...
    for (auto &job : Simulator::jobs) {
        if (pthread_join(job.first, &status) != 0) {
            err = "a4w22: " + job.second->jobName + " pthread_join error()";
            cerr << err << endl;
        }
    }
    ...
    if (pthread_join(monitorId, &status) != 0) {
        err = "a4w22: monitor pthread_join error()";
        cerr << err << endl;
    }
...
void Simulator::terminate(const string &err) {
    for (auto &job : Simulator::jobs) {
        if (pthread_cancel(job.first) != 0)
            cerr << "a4w22: job " << job.second->jobName << " pthread_cancel error()" << endl;
    }

    if (pthread_cancel(monitorId) != 0)
        cerr << "a4w22: monitor pthread_cancel error()" << endl;
    ...
    if (tokens.front() == "resources") {
        ...
        for (auto it = tokens.begin() + 1; it != tokens.end(); ++it) {
            if (Simulator::manager->getNumberType() == NRES_TYPES) {
                err = "a4w22: maximum number of resources type reached";
                terminate(err);
            }

            if (Simulator::manager->addResource(*it)) {
                err = "a4w22: duplicated resource type: " + *it;
                terminate(err);
            }
        }
...
void Simulator::createJob(vector<string> &args) {
    string err;
    if (numJobs >= NJOBS) {
        err = "a4w22: maximum number of jobs reached";
        terminate(err);
    }
    ...
    if (hasJob(info->jobName)) {
        err = "a4w22: duplicated job: " + info->jobName;
        delete info;
        info = nullptr;
        terminate(err);
    }
    ...
    if (resourceCheck(info->need)) {
        err = "a4w22: job " + info->jobName + " requires a resource type not existed";
        delete info;
        info = nullptr;
        terminate(err);
    }
    ...
    if (pthread_create(&jobId, nullptr, (ThreadFuncPtr)&Job::start, (void *)info) != 0) {
        err = "a4w22: job " + info->jobName + " pthread_create() error";
        delete info;
        info = nullptr;
        terminate(err);
    } else {
