...
static void _Task_Motor( void *pvParameters ){
	...
	while(1){
		...
		long currentposition_in_steps = 0;
		float rotational_speed = 0;
		float rotational_acceleration = 0;
		float rotational_deceleration = 0;

		memset( (char *)&read_motor_parameters_from_queue, 0, sizeof(read_motor_parameters_from_queue) );
		
		xQueueReceive( xQueue_FIFO1, &read_motor_parameters_from_queue, portMAX_DELAY );

		currentposition_in_steps = read_motor_parameters_from_queue->currentposition_in_steps;
		rotational_speed = read_motor_parameters_from_queue->rotational_speed;
		rotational_acceleration = read_motor_parameters_from_queue->rotational_acceleration;
		rotational_deceleration = read_motor_parameters_from_queue->rotational_deceleration;
		...
		Stepper_setCurrentPositionInSteps(currentposition_in_steps);
		Stepper_setSpeedInStepsPerSecond(rotational_speed);
		Stepper_setAccelerationInStepsPerSecondPerSecond(rotational_acceleration);
		Stepper_setDecelerationInStepsPerSecondPerSecond(rotational_deceleration);

		for (int i = 0; i < sequenceIndex; ++i) {
			if (Stepper_getCurrentPositionInSteps() == 0)
...
static void _Task_Emerg_Stop( void *pvParameters ){
	...
	while(1) {
		...
		if(pressedCount >= 3) {
			...
			memset((char *)&positionSequence, 0, sizeof(positionSequence));

			while(1) {
				if (Stepper_motionComplete() && !isDisabled) {
					Stepper_disableMotor();
					isDisabled = 1;
				}
