...
def make_tree(freq_table, inclEOF=True):
    ...
    Check workbook and lecture 18

    """

    trees = []  # initial list of frequencies, and then turn it into binary tree
...
def make_freq_table(stream):
    ...
    unit testing -> fake stream, test without using an actual file
    - create a buffer (byte array of size 512), read into that from the file 512 bytes at a time
    - iterate all the byte and the buffer, and count the number of times every byte value see (byte value, from 0 to 255)
    - more frequent byte value will be given the shorter bit representation for encoding and encoding tree
