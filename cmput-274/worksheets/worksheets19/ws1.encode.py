...
def encode(exStr):
    ...
    print(freqs)
    tree = make_tree(freqs)     # Use EOF, if it comes up
    print(tree)
    eTable = make_encoding_table(tree)
    print(eTable)
    ...
    for i in list(exStr):
        if ord(i) in eTable:
            if Debug:
                print(ord(i), eTable[ord(i)])
            for b in eTable[ord(i)]:
                if b == True:
            ...
            print("Error 2")
        if Debug:
            print(i, ''.join(encodedList))   # Expensive, but just for debug
    encodedString = ''.join(encodedList)
    print("Encoding: ", encodedString)
    oLen = len(exStr) * 8
    eLen = len(encodedString)
    print("   Ratio:  %f = %d / %d (bits)" % (float(oLen)/eLen, oLen, eLen))
    return

def testmain():
    encode("hellohello")
    line, c = safe_input(prompt="Input String: ")
    while c:
        encode(line)
        line, c = safe_input(prompt="Input String: ")
