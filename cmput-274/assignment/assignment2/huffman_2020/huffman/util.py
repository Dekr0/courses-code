...
def decode_byte(tree, bitreader):
    ...
    # using recursion to decode / read the tree
    ...
    if isinstance(tree, huffman.TreeLeaf):
        ...
        # If reaching the leaf node whose value is None (EOF), stop
        # reading the compressed stream as well as stop writing
        # uncompress bits

    bit = bitreader.readbit()

    if bit:
        # if read a bit of 1, go right
        r_subtree = tree.getRight()
        return decode_byte(r_subtree, bitreader)

    # go left if read a bit of 0
...
def compress(tree, uncompressed, compressed):
    ...
    # generate an encoding table of the compressed stream
    ...
    while not EOF:
        try:
            ...
            # write the EOF (None) into the compressed file, and finish both
            # read and write
            ...
            # check whether there are any partially-written bytes remaining
            # at the end of the file. If none, the mod of 8 of total number of
            # written bits should be 0
            remain = total_bits % 8
            if remain > 0:
