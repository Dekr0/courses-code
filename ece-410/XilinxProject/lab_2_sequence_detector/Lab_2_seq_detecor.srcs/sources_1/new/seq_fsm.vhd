...
-- Remember, you want to add this component here and then use it later when you 
-- wish to have the divided clock by a factor of 62500000
...
-- port map the clk_divider here
    divider : clk_divider port map(clk_in => clk, clk_out => clk_o);
---------------------------------------------
    
    -- the process below uses the 'clk' i.e. the undivided clock , i.e. the clock 
    -- signal from the entity.
    -- you can replace it with the divided clock signal later on when you add the 
    -- 'clk_divider' component.
    -- same way, you will need to change the clock signal name in the 'elsif'
    -- statement inside the process below, later on!
