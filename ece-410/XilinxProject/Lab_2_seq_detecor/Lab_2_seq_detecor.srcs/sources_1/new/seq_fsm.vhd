...
-- port map the clk_divider here
    -- fsm_clk_divider: clk_divider port map(clk_in => clk);
---------------------------------------------
    ...
    process(clk) -- "clk" gets replaced with the divided clock later on!
        begin
           if(reset='1')then
                state_reg <= S0;
           elsif(clk'event and clk = '1')then  -- "clk" gets replaced with the divided clock later on!
